...
const pepperSound = new Audio("assets/pepper.mp3"); // 👈 1. BİBƏR SƏSİ ƏLAVƏ OLUNDU
...
/* FON MUSİQİSİNİ ZƏMANƏTLİ ÇALMA */
...
function startGame(){
  ...
  homeBgm.pause();
  pepperSound.pause(); // 👈 Oyuna başlayanda köhnə bibər səsini kəs
...
function goHome(){
  ...
  pepperSound.pause(); // 👈 Menuya qayıdanda bibər səsini kəs
  highscoreSound.pause();
  gameOverSound.pause(); 
  playHomeMusic();
...
function endGame(){
  ...
  homeBgm.pause();
  pepperSound.pause(); // 👈 Oyun bitəndə bibər səsini kəs
  highscoreSound.pause();
...
function createItem(){
  ...
  if (rand < 0.18) {
    ...
    /* 62% Ehtimalla Şirniyyat */
    item.innerText = sweets[Math.floor(Math.random() * sweets.length)];
  ...
  const fall = setInterval(() => {
    ...
    if(caught){
      if(item.dataset.type === "bomb"){
        ...
        // 👈 2. BİBƏRƏ DƏYƏNDƏ SƏS EFFEKTİNİN ÇALINMASI
        ...
        if(score <= 0) {
          ...
          clearInterval(fall);
          item.remove();
          endGame(); /* XAL 0 OLANDA OYUN BİTİR */
          return;
        }
        scoreText.innerText = score;

        // HUD Score reaksiyası
        ...
        // Havada böyük fırlanan crazy pop-up
        const popup = document.createElement("div");
...
/* ANA SƏHİFƏ FONDAN UÇAN KONFETLƏR */
